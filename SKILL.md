...
```bash
# 3.1 确认编译环境
# 用户会指定编译服务器、架构、交叉编译工具链前缀
# 先诊断编译工具链是否完整：
which <cross_compile_prefix>gcc
which <cross_compile_prefix>ld

# 3.2 如有缺失，先修复编译环境
# 常见缺失：有gcc但无binutils → 安装对应包或使用语法检查

# 3.3 逐个patch验证（关键：不是全部打完后验证！）
# 方式：逐个checkout每个commit，分别编译验证

# 对于内核类项目：
make ARCH=<arch> CROSS_COMPILE=<prefix> <affected_module_dir>/
