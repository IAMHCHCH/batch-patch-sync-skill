...
首先询问用户选择**本地编译**还是**远程服务器编译**：

- **优先检测本地环境**：检查本地是否已安装所需的交叉编译工具链
  ```bash
  which <cross_compile_prefix>gcc && which <cross_compile_prefix>ld && echo "本地编译可用" || echo "本地编译不可用"
  ```
- 如果本地工具链完整，推荐用户优先使用本地编译（更高效、无网络依赖）
- 如果本地缺少工具链，询问用户：a) 在本地安装工具链 还是 b) 使用远程编译服务器
...
验证本地工具链：
```bash
which <cross_compile_prefix>gcc && which <cross_compile_prefix>ld
```
如有缺失，提示用户安装命令（如 `apt install gcc-aarch64-linux-gnu binutils-aarch64-linux-gnu`）。

#### 步骤0c: 远程服务器编译（备选）

如果本地不支持且用户选择远程服务器，**必须询问以下信息（缺一不可）**：
- 远程编译服务器IP地址
- SSH登录用户名
- 认证方式（密码 / SSH密钥 / 已配置免密）
- 目标编译架构（如 arm64、x86_64）
...
**连接验证**: 确认免密（或使用提供的密码）能正常连接后，再开始同步工作。
...
**前提**: 已在规则0中确认编译方式（本地/远程）并获取了完整的环境信息（架构、工具链前缀、目标模块目录）。
...
### 开始前（阶段0-1）
- [ ] **已确认编译方式**（本地 / 远程服务器）
- [ ] **编译工具链已验证完整**（本地 `which gcc ld` 或远程 `ssh ... which gcc ld`）
- [ ] **（如为远程）SSH免密已配置并验证通过**，或用户已提供密码
